...
** Api Example **
    ...
    foreach ($locations as $location) {
        $latlng = get_post_meta($location->ID, "latlng")
        $marker = new TkMapMarker();
        $marker->setCoords($latlng[0], $latlng[1]);
        $marker->addInfoWindow($location->description);
        $map->addMarker($marker);
    }
    
    $map->setHeight(600);
    $map->setPlaceholderImage("/wp-content/uploads/maps-placeholder.jpg");
    
    return $map->render();

Check TkMap and TkMapMarker classes to see what else they can do
